...
document.getElementById("release-notes").innerHTML=update.releaseNotes.map(({version,note:html})=>`<section><h3>Version ${version}</h3><div>${html.replace(/<\/?h([1-3])/g,(m,level)=>m.replace(level,Number(level)+3))}</div></section>`).join("\n");
document.querySelectorAll("a").forEach(a=>{a.target="_blank"});
document.querySelectorAll("h3,h4,h5,h6").forEach(h=>{if(h.textContent.trim().toLowerCase()==="what's changed")h.remove()});
...
document.getElementById("ignore-button").addEventListener("click",()=>{if(confirm("Are you sure you want to ignore this update?"))VesktopUpdaterNative.ignoreUpdate()});
VesktopUpdaterNative.onProgress(percent=>(downloadProgress.value=percent));
VesktopUpdaterNative.onError(message=>{updateDialog.closedBy="any";errorText.textContent=`Error: ${message}`;installingDialog.close();updateDialog.classList.remove("hidden")});
