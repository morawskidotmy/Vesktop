...
document.getElementById("release-notes").innerHTML = update.releaseNotes
    .map(
        ({ version, note: html }) =>
            `<section><h3>Version ${version}</h3><div>${html.replace(/<\/?h([1-3])/g, (m, level) => m.replace(level, Number(level) + 3))}</div></section>`
    )
    .join("\n");
...
document.querySelectorAll("h3,h4,h5,h6").forEach(h => {
    if (h.textContent.trim().toLowerCase() === "what's changed") h.remove();
});
...
document.getElementById("update-button").addEventListener("click", () => {
    ...
    if (navigator.platform.startsWith("Linux")) document.getElementById("linux-note").classList.remove("hidden");
...
document.getElementById("ignore-button").addEventListener("click", () => {
    if (confirm("Are you sure you want to ignore this update?")) VesktopUpdaterNative.ignoreUpdate();
});
